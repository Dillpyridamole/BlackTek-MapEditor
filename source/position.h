...
class Position {
  public:
	// We use int since it's the native machine type and can be several times
	// faster than the other integer types in most cases, also, the position may
...
inline Position abs(const Position &position) {
	return Position(std::abs(position.x), std::abs(position.y),
					std::abs(position.z));
}
