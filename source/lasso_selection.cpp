...
void LassoSelection::addPoint(int x, int y)
{
    ...
    if (!m_path.empty()) {
        if (m_path.back().distanceTo(newPoint) < m_minPointDistance) {
    ...
    // Update simplified path for rendering
    if (m_path.size() > 2) {
        simplifyPath();
    } else {
...
void LassoSelection::simplifyPath()
{
    ...
    rdpSimplify(m_path, 0, static_cast<int>(m_path.size()) - 1, m_simplifyTolerance, keep);
...
void LassoSelection::rdpSimplify(const std::vector<LassoPoint>& points, int start, int end,
                                              ...
                                              const LassoPoint& lineEnd) const
{
    ...
    double cross = std::abs((point.x - lineStart.x) * dy - (point.y - lineStart.y) * dx);
...
void LassoSelection::scanlineFill(int floor, std::vector<Position>& tiles) const
{
    // Use simplified path for the polygon
    const std::vector<LassoPoint>& poly = m_simplifiedPath.empty() ? m_path : m_simplifiedPath;
