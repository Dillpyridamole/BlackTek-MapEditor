...
{
public:
    LootListBox(wxWindow* parent, wxWindowID id, HuntingCalculatorWindow* calculator);
...
public:
    ...
    void SetArea(int startX, int startY, int startZ, int endX, int endY, int endZ);
    ...
    double CalculateTimeForProbability(const AggregatedLoot& item, double probability) const;
...
private:
    ...
    bool LoadMonsterFromXML(const std::string& filepath, HuntingMonsterData& data);
    void ParseLootXML(pugi::xml_node lootNode, std::vector<HuntingMonsterData::LootItem>& lootList);
    bool LoadMonsterFromLua(const std::string& filepath, HuntingMonsterData& data);
    void ParseLootLua(const std::string& content, std::vector<HuntingMonsterData::LootItem>& lootList);
    ...
    std::map<std::string, HuntingMonsterData> m_monsterDatabase;
    ...
    // Selection mode (true = use selected tiles directly, false = use coordinates)
    ...
    std::vector<CachedMonsterData> m_cachedMonsters;  // Only monsters, not all tiles
