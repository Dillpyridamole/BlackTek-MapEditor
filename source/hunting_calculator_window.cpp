...
// ============================================================================
// Optimization: Static item name to ID cache
// Avoids O(50K) iteration when looking up items by name
// ============================================================================
static std::unordered_map<std::string, uint16_t> s_itemNameToIdCache;
static bool s_itemCacheBuilt = false;

static void BuildItemNameCache() {
	if (s_itemCacheBuilt)
		return;

	s_itemNameToIdCache.reserve(30000); // Pre-reserve for performance

	for (uint16_t id = 100; id < 50000; ++id) {
	...
	s_itemCacheBuilt = true;
}

static uint16_t LookupItemIdByName(const std::string &name) {
	if (!s_itemCacheBuilt) {
		BuildItemNameCache();
	}
...
void MonsterListBox::OnDrawItem(wxDC &dc, const wxRect &rect, size_t n) const {
	if (!m_monsters || n >= m_monsters->size())
		return;
...
wxCoord MonsterListBox::OnMeasureItem(size_t n) const { return 36; }
...
wxCoord LootListBox::OnMeasureItem(size_t n) const { return 36; }

void LootListBox::OnRightClick(wxMouseEvent &event) {
	if (!m_loot || m_loot->empty())
		return;
...
void HuntingCalculatorWindow::LoadMonstersFromArea() {
	...
	if (m_useSelection) {
		...
		if (m_cacheValid && !m_cachedMonsters.empty()) {
			...
			// Use the cached floor (which was detected from the selection in
			// CacheSelectionTiles)
			int currentFloor = m_cachedCurrentFloor;

			// Copy tiles to local vector first to avoid issues with selection
			// modification
...
void HuntingCalculatorWindow::ParseLootLua(
	...
	while (pos < lootSection.size()) {
		...
		while (braceEnd < lootSection.size() && braceCount > 0) {
			if (lootSection[braceEnd] == '{')
				braceCount++;
			else if (lootSection[braceEnd] == '}')
				braceCount--;
			braceEnd++;
		}
		...
		if (std::regex_search(block, idMatch, lootStringRegex)) {
			...
			// Optimization: Use cached item name lookup instead of O(50K) loop
			lootItem.id = LookupItemIdByName(lootItem.name);
			hasId = true;
...
void HuntingCalculatorWindow::CalculateResults() {
	...
	for (auto &monster : m_monstersInArea) {
		...
		if (monster.respawnTime <= 0.0)
			monster.respawnTime = 1.0; // Minimum 1 second
		...
		if (effectiveTimePerKill < 1.0)
			effectiveTimePerKill = 1.0;
