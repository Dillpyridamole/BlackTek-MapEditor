...
MapPropertiesWindow::MapPropertiesWindow(wxWindow *parent, MapTab *view,
										 Editor &editor)
	: wxDialog(parent, wxID_ANY, "Map Properties", wxDefaultPosition,
			   wxSize(300, 200), wxRESIZE_BORDER | wxCAPTION),
	...
	description_ctrl =
		newd wxTextCtrl(this, wxID_ANY, wxstr(map.getMapDescription()),
	...
	protocol_choice->SetStringSelection(
		wxstr(g_gui.GetCurrentVersion().getName()));
	...
	grid_sizer->Add(newd wxStaticText(this, wxID_ANY, "Map Dimensions"));
	{
		...
		subsizer->Add(width_spin = newd wxSpinCtrl(
						  this, wxID_ANY, wxstr(i2s(map.getWidth())),
						  wxDefaultPosition, wxDefaultSize, wxSP_ARROW_KEYS,
						  rme::MapMinWidth, rme::MapMaxWidth),
					  wxSizerFlags(1).Expand());
		subsizer->Add(height_spin = newd wxSpinCtrl(
						  this, wxID_ANY, wxstr(i2s(map.getHeight())),
						  wxDefaultPosition, wxDefaultSize, wxSP_ARROW_KEYS,
						  rme::MapMinHeight, rme::MapMaxHeight),
					  wxSizerFlags(1).Expand());
	...
	grid_sizer->Add(house_filename_ctrl = newd wxTextCtrl(
						this, wxID_ANY, wxstr(map.getHouseFilename())),
					1, wxEXPAND);

	grid_sizer->Add(newd wxStaticText(this, wxID_ANY, "External Spawnfile"));

	grid_sizer->Add(spawn_filename_ctrl = newd wxTextCtrl(
						this, wxID_ANY, wxstr(map.getSpawnFilename())),
					1, wxEXPAND);
	...
	subsizer->Add(newd wxButton(this, wxID_CANCEL, "Cancel"),
				  wxSizerFlags(1).Center());
	topsizer->Add(subsizer, wxSizerFlags(0).Center().Border(
								wxLEFT | wxRIGHT | wxBOTTOM, 20));
	...
	ClientVersion *current_version =
		ClientVersion::get(map.getVersion().client);
...
void MapPropertiesWindow::UpdateProtocolList() {
	...
	if (g_settings.getInteger(Config::USE_OTBM_4_FOR_ALL_MAPS)) {
		...
		ClientVersionList protocols =
			ClientVersion::getAllForOTBMVersion(map_version);
		for (ClientVersionList::const_iterator p = protocols.begin();
			 p != protocols.end(); ++p)
...
struct MapConversionContext {
	...
	void operator()(Map &map, Tile *tile, long long done) {
		if (tile->creature) {
			CreatureMap::iterator f =
				creature_types.find(tile->creature->getName());
			if (f == creature_types.end()) {
				CreatureInfo info = {tile->creature->getName(),
									 tile->creature->isNpc(),
...
void MapPropertiesWindow::OnClickOK(wxCommandEvent &WXUNUSED(event)) {
	...
	new_ver.client =
		ClientVersion::get(nstr(protocol_choice->GetStringSelection()))
			->getID();
	if (ver.Contains("0.5.0")) {
	...
	if (new_ver.client != old_ver.client) {
		if (g_gui.GetOpenMapCount() > 1) {
			g_gui.PopupDialog(
				this, "Error",
				"You can not change editor version with multiple maps open",
				wxOK);
			return;
		}
		wxString error;
		...
		if (new_ver.client < old_ver.client) {
			int ret = g_gui.PopupDialog(
				this, "Notice",
				"Converting to a previous version may have serious "
				"side-effects, are you sure you want to do this?",
				wxYES | wxNO);
			if (ret != wxID_YES) {
				return;
			}
			...
			if (!g_gui.LoadVersion(new_ver.client, error, warnings)) {
				...
				g_gui.PopupDialog(
					...
					"Could not convert map. The map will now be closed.", wxOK);

				EndModal(0);
				return;
			}

			// Remove all creatures that were present are present in the new
			// version
			for (MapConversionContext::CreatureMap::iterator cs =
					 conversion_context.creature_types.begin();
			...
			if (conversion_context.creature_types.size() > 0) {
				int add = g_gui.PopupDialog(
					...
					"There were creatures on the old version that are not "
					"present in this and were on the map, do you want to add "
					"them to this version as well?",
					wxYES | wxNO);
				if (add == wxID_YES) {
					for (MapConversionContext::CreatureMap::iterator cs =
							 conversion_context.creature_types.begin();
						...
						g_creatures.addCreatureType(info.name, info.is_npc,
													info.outfit);
					}
				}
			}
...
ImportMapWindow::ImportMapWindow(wxWindow *parent, Editor &editor)
	: wxDialog(parent, wxID_ANY, "Import Map", wxDefaultPosition,
			   wxSize(420, 315)),
	  editor(editor) {
	...
	// File
	tmpsizer = newd wxStaticBoxSizer(
		new wxStaticBox(this, wxID_ANY, "Map File"), wxHORIZONTAL);
	file_text_field = newd wxTextCtrl(tmpsizer->GetStaticBox(), wxID_ANY, "",
									  wxDefaultPosition, wxSize(300, 23));
	tmpsizer->Add(file_text_field, 0, wxALL, 5);
	wxButton *browse_button =
		newd wxButton(tmpsizer->GetStaticBox(), MAP_WINDOW_FILE_BUTTON,
					  "Browse...", wxDefaultPosition, wxSize(80, 23));
	tmpsizer->Add(browse_button, 0, wxALL, 5);
	sizer->Add(tmpsizer, 1, wxEXPAND | wxLEFT | wxRIGHT | wxTOP, 5);

	// Import offset
	tmpsizer = newd wxStaticBoxSizer(
		new wxStaticBox(this, wxID_ANY, "Import Offset"), wxHORIZONTAL);
	tmpsizer->Add(
		newd wxStaticText(tmpsizer->GetStaticBox(), wxID_ANY, "Offset X:"), 0,
		wxALL | wxEXPAND, 5);
	x_offset_ctrl = newd wxSpinCtrl(
		tmpsizer->GetStaticBox(), wxID_ANY, wxEmptyString, wxDefaultPosition,
		wxSize(60, 23), wxSP_ARROW_KEYS, -rme::MapMaxHeight, rme::MapMaxHeight);
	tmpsizer->Add(x_offset_ctrl, 0, wxALL, 5);
	tmpsizer->Add(
		newd wxStaticText(tmpsizer->GetStaticBox(), wxID_ANY, "Offset Y:"), 0,
		wxALL, 5);
	y_offset_ctrl = newd wxSpinCtrl(
		tmpsizer->GetStaticBox(), wxID_ANY, wxEmptyString, wxDefaultPosition,
		wxSize(60, 23), wxSP_ARROW_KEYS, -rme::MapMaxHeight, rme::MapMaxHeight);
	tmpsizer->Add(y_offset_ctrl, 0, wxALL, 5);
	tmpsizer->Add(
		newd wxStaticText(tmpsizer->GetStaticBox(), wxID_ANY, "Offset Z:"), 0,
		wxALL, 5);
	z_offset_ctrl = newd wxSpinCtrl(
		tmpsizer->GetStaticBox(), wxID_ANY, wxEmptyString, wxDefaultPosition,
	...
	// House options
	tmpsizer = newd wxStaticBoxSizer(
		new wxStaticBox(this, wxID_ANY, "House Import Behaviour"), wxVERTICAL);
	house_options =
		newd wxChoice(tmpsizer->GetStaticBox(), wxID_ANY, wxDefaultPosition,
					  wxDefaultSize, house_choices);
	...
	// Spawn options
	tmpsizer = newd wxStaticBoxSizer(
		new wxStaticBox(this, wxID_ANY, "Spawn Import Behaviour"), wxVERTICAL);
	spawn_options =
		newd wxChoice(tmpsizer->GetStaticBox(), wxID_ANY, wxDefaultPosition,
					  wxDefaultSize, spawn_choices);
...
void ImportMapWindow::OnClickBrowse(wxCommandEvent &WXUNUSED(event)) {
	wxFileDialog dialog(this, "Import...", "", "", "*.otbm",
	...
	if (Validate() && TransferDataFromWindow()) {
		...
		if (!fn.FileExists()) {
			g_gui.PopupDialog(this, "Error",
							  "The specified map file doesn't exist", wxOK);
			return;
		}
		...
		editor.importMap(fn, x_offset_ctrl->GetValue(),
						 y_offset_ctrl->GetValue(), z_offset_ctrl->GetValue(),
...
ExportMiniMapWindow::ExportMiniMapWindow(wxWindow *parent, Editor &editor)
	: wxDialog(parent, wxID_ANY, "Export Minimap", wxDefaultPosition,
			   wxSize(400, 300)),
	  editor(editor) {
	...
	// Error field
	error_field = newd wxStaticText(this, wxID_VIEW_DETAILS, "",
	...
	// Output folder
	directory_text_field =
		newd wxTextCtrl(this, wxID_ANY, "", wxDefaultPosition, wxDefaultSize);
	directory_text_field->Bind(wxEVT_KEY_UP,
							   &ExportMiniMapWindow::OnDirectoryChanged, this);
	directory_text_field->SetValue(
		wxString(g_settings.getString(Config::MINIMAP_EXPORT_DIR)));
	tmpsizer = newd wxStaticBoxSizer(wxHORIZONTAL, this, "Output Folder");
	tmpsizer->Add(directory_text_field, 1, wxALL, 5);
	tmpsizer->Add(newd wxButton(this, MAP_WINDOW_FILE_BUTTON, "Browse"), 0,
				  wxALL, 5);
	...
	file_name_text_field =
		newd wxTextCtrl(this, wxID_ANY, mapName.BeforeLast('.'),
						wxDefaultPosition, wxDefaultSize);
	file_name_text_field->Bind(wxEVT_KEY_UP,
							   &ExportMiniMapWindow::OnFileNameChanged, this);
	...
	format_options = new wxChoice(this, wxID_ANY, wxDefaultPosition,
								  wxDefaultSize, format_choices);
	...
	floor_options = newd wxChoice(this, wxID_ANY, wxDefaultPosition,
								  wxDefaultSize, choices);
	floor_number = newd wxSpinCtrl(this, wxID_ANY, i2ws(rme::MapGroundLayer),
								   wxDefaultPosition, wxDefaultSize,
								   wxSP_ARROW_KEYS, rme::MapMinLayer,
								   rme::MapMaxLayer, rme::MapGroundLayer);
	...
	// OK/Cancel buttons
	tmpsizer = newd wxBoxSizer(wxHORIZONTAL);
	tmpsizer->Add(ok_button = newd wxButton(this, wxID_OK, "OK"),
				  wxSizerFlags(1).Center());
	tmpsizer->Add(newd wxButton(this, wxID_CANCEL, "Cancel"),
				  wxSizerFlags(1).Center());
...
void ExportMiniMapWindow::OnClickBrowse(wxCommandEvent &WXUNUSED(event)) {
	wxDirDialog dialog(NULL, "Select the output folder", "",
...
void ExportMiniMapWindow::OnClickOK(wxCommandEvent &WXUNUSED(event)) {
	...
	auto format =
		static_cast<MinimapExportFormat>(format_options->GetSelection());
...
FindDialog::FindDialog(wxWindow *parent, wxString title)
	...
	search_field = newd KeyForwardingTextCtrl(this, JUMP_DIALOG_TEXT, "",
											  wxDefaultPosition, wxDefaultSize,
											  wxTE_PROCESS_ENTER);
	...
	stdsizer->Add(newd wxButton(this, wxID_CANCEL, "Cancel"),
				  wxSizerFlags(1).Center());
		...
		if (item_list->GetItemCount() > 0) {
			...
			if (n == wxNOT_FOUND)
				n = 0;
					 ...
					 n - amount < n) // latter is needed for unsigned overflow
				n -= amount;
			else
				n = 0;
	...
	if (item_list->GetItemCount() > 0) {
		...
		if (!brush) {
			...
			std::string search_string =
				as_lower_str(nstr(search_field->GetValue()));
			bool do_search = (search_string.size() >= 2);

			if (do_search) {
				const BrushMap &map = g_brushes.getMap();
				for (BrushMap::const_iterator iter = map.begin();
					 iter != map.end(); ++iter) {
					const Brush *brush = iter->second;
					if (as_lower_str(brush->getName()).find(search_string) ==
						std::string::npos)
						continue;
				...
				if (!result_brush) {
					...
					for (int id = 0; id <= g_items.getMaxID(); ++id) {
						...
						if (as_lower_str(raw_brush->getName())
								.find(search_string) == std::string::npos)
							continue;
...
void FindBrushDialog::RefreshContentsInternal() {
	...
	if (do_search) {
		...
		for (BrushMap::const_iterator iter = brushes_map.begin();
			 iter != brushes_map.end(); ++iter) {
			const Brush *brush = iter->second;
...
FindDialogListBox::FindDialogListBox(wxWindow *parent, wxWindowID id)
	: wxVListBox(parent, id, wxDefaultPosition, wxDefaultSize, wxLB_SINGLE),
	  cleared(false), no_matches(false) {
	Clear();
}
...
void FindDialogListBox::OnDrawItem(wxDC &dc, const wxRect &rect,
								   size_t n) const {
	if (no_matches) {
		dc.DrawText("No matches for your search.", rect.GetX() + 40,
					rect.GetY() + 6);
	} else if (cleared) {
		dc.DrawText("Please enter your search string.", rect.GetX() + 40,
					rect.GetY() + 6);
	} else {
		ASSERT(n < brushlist.size());
		Sprite *spr = g_gui.gfx.getSprite(brushlist[n]->getLookID());
		if (spr) {
			spr->DrawTo(&dc, SPRITE_SIZE_32x32, rect.GetX(), rect.GetY(),
			...
			auto creatureSprite =
				g_gui.gfx.getCreatureSprite(creatureType->outfit.lookType);
			if (creatureSprite) {
		...
		dc.DrawText(wxstr(brushlist[n]->getName()), rect.GetX() + 40,
					rect.GetY() + 6);
	}
}
...
SortableListBox::SortableListBox(wxWindow *parent, wxWindowID id,
								 const wxPoint &pos, const wxSize &size)
	: wxListBox(parent, id, pos, size, 0, nullptr,
...
void SortableListBox::Sort() {
	...
	wxASSERT_MSG(GetClientDataType() != wxClientData_Object,
				 "Sorting a list with data of type wxClientData_Object is "
...
ObjectPropertiesWindowBase::ObjectPropertiesWindowBase(
	wxWindow *parent, wxString title, const Map *map, const Tile *tile,
	Item *item, wxPoint position /* = wxDefaultPosition */)
	: wxDialog(parent, wxID_ANY, title, position, wxSize(600, 400),
			   wxCAPTION | wxCLOSE_BOX | wxRESIZE_BORDER),
	  edit_map(map), edit_tile(tile), edit_item(item), edit_creature(nullptr),
	  edit_spawn(nullptr) {
	////
}
...
ObjectPropertiesWindowBase::ObjectPropertiesWindowBase(
	wxWindow *parent, wxString title, const Map *map, const Tile *tile,
	Creature *creature, wxPoint position /* = wxDefaultPosition */)
	: wxDialog(parent, wxID_ANY, title, position, wxSize(600, 400),
			   wxCAPTION | wxCLOSE_BOX | wxRESIZE_BORDER),
	  edit_map(map), edit_tile(tile), edit_item(nullptr),
	  edit_creature(creature), edit_spawn(nullptr) {
	////
}

ObjectPropertiesWindowBase::ObjectPropertiesWindowBase(
	wxWindow *parent, wxString title, const Map *map, const Tile *tile,
	Spawn *spawn, wxPoint position /* = wxDefaultPosition */)
	: wxDialog(parent, wxID_ANY, title, position, wxSize(600, 400),
			   wxCAPTION | wxCLOSE_BOX | wxRESIZE_BORDER),
	  edit_map(map), edit_tile(tile), edit_item(nullptr),
	  edit_creature(nullptr), edit_spawn(spawn) {
	////
}
...
EVT_BUTTON(EDIT_TOWNS_SELECT_TEMPLE,
...
EditTownsDialog::EditTownsDialog(wxWindow *parent, Editor &editor)
	...
	for (TownMap::const_iterator town_iter = map.towns.begin();
		 town_iter != map.towns.end(); ++town_iter) {
	...
	// Town list
	town_listbox = newd wxListBox(this, EDIT_TOWNS_LISTBOX, wxDefaultPosition,
								  wxSize(240, 100));
	sizer->Add(town_listbox, 1, wxEXPAND | wxTOP | wxLEFT | wxRIGHT, 10);

	tmpsizer = newd wxBoxSizer(wxHORIZONTAL);
	tmpsizer->Add(newd wxButton(this, EDIT_TOWNS_ADD, "Add"), 0, wxTOP, 5);
	tmpsizer->Add(remove_button =
					  newd wxButton(this, EDIT_TOWNS_REMOVE, "Remove"),
				  0, wxRIGHT | wxTOP, 5);
	...
	name_field =
		newd wxTextCtrl(this, wxID_ANY, "", wxDefaultPosition, wxSize(190, 20),
						0, wxTextValidator(wxFILTER_ASCII, &town_name));
	tmpsizer->Add(name_field, 2, wxEXPAND | wxLEFT | wxBOTTOM, 5);

	id_field =
		newd wxTextCtrl(this, wxID_ANY, "", wxDefaultPosition, wxSize(40, 20),
						0, wxTextValidator(wxFILTER_NUMERIC, &town_id));
	id_field->Enable(false);
	tmpsizer->Add(id_field, 1, wxEXPAND | wxLEFT | wxRIGHT | wxBOTTOM, 5);
	sizer->Add(tmpsizer, 0, wxEXPAND | wxALL, 10);

	// Temple position
	temple_position = newd PositionCtrl(this, "Temple Position", 0, 0, 0,
										map.getWidth(), map.getHeight());
	select_position_button =
		newd wxButton(this, EDIT_TOWNS_SELECT_TEMPLE, "Go To");
	temple_position->Add(select_position_button, 0, wxLEFT | wxRIGHT | wxBOTTOM,
						 5);
...
EditTownsDialog::~EditTownsDialog() {
	for (std::vector<Town *>::iterator town_iter = town_list.begin();
		 town_iter != town_list.end(); ++town_iter) {
		delete *town_iter;
	}
}
	...
	if (town_list.size() > 0) {
		if (id_field->GetValue().ToLong(&tmplong)) {
			...
			if (old_town) {
				...
				// old_town->getName().c_str()); printf("New values
				// %d:%s:%d:%d:%d\n", town_id, town_name.c_str(), templepos.x,
				// templepos.y, templepos.z);
...
void EditTownsDialog::OnClickSelectTemplePosition(
	...
	if (templepos.isValid() && templepos.x > 0 && templepos.y > 0) {
		...
		g_gui.SetStatusText(
			"Invalid temple position - please set a valid position first");
	}
}
	...
	if (id_field->GetValue().ToLong(&tmplong)) {
		...
		for (const auto &pair : map.houses) {
			if (pair.second->townid == town->getID()) {
				g_gui.PopupDialog(this, "Error",
								  "You cannot delete a town which still has "
...
void EditTownsDialog::OnClickOK(wxCommandEvent &WXUNUSED(event)) {
	...
	if (Validate() && TransferDataFromWindow()) {
		...
		for (std::vector<Town *>::iterator town_iter = town_list.begin();
			...
			if (town->getName() == "") {
				g_gui.PopupDialog(this, "Error",
								  "You can't have a town with an empty name.",
								  wxOK);
				return;
			}
			if (!town->getTemplePosition().isValid() ||
				...
				wxString msg;
				msg << "The town " << wxstr(town->getName())
					<< " has an invalid temple position.";
...
GotoPositionDialog::GotoPositionDialog(wxWindow *parent, Editor &editor)
	: wxDialog(parent, wxID_ANY, "Go To Position", wxDefaultPosition,
			   wxDefaultSize),
	  editor(editor) {
	const Map &map = editor.getMap();
	...
	posctrl = newd PositionCtrl(this, "Destination", map.getWidth() / 2,
								map.getHeight() / 2, rme::MapGroundLayer,
								map.getWidth(), map.getHeight());
