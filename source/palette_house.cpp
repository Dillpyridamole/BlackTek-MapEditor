...
EVT_LISTBOX_DCLICK(PALETTE_HOUSE_LISTBOX,
...
EVT_TOGGLEBUTTON(PALETTE_HOUSE_BRUSH_BUTTON,
				 HousePalettePanel::OnClickHouseBrushButton)
EVT_TOGGLEBUTTON(PALETTE_HOUSE_SELECT_EXIT_BUTTON,
				 HousePalettePanel::OnClickSelectExitButton)
END_EVENT_TABLE()

HousePalettePanel::HousePalettePanel(wxWindow *parent, wxWindowID id)
	: PalettePanel(parent, id), map(nullptr), do_resize_on_display(true),
	...
	town_choice =
		newd wxChoice(this, PALETTE_HOUSE_TOWN_CHOICE, wxDefaultPosition,
					  wxDefaultSize, (int)0, (const wxString *)nullptr);
	...
	tmpsizer->Add(add_house_button =
					  newd wxButton(this, PALETTE_HOUSE_ADD_HOUSE, "Add",
									wxDefaultPosition, wxSize(50, -1)),
				  sizerFlags);
	tmpsizer->Add(edit_house_button =
					  newd wxButton(this, PALETTE_HOUSE_EDIT_HOUSE, "Edit",
									wxDefaultPosition, wxSize(50, -1)),
				  sizerFlags);
	tmpsizer->Add(remove_house_button =
					  newd wxButton(this, PALETTE_HOUSE_REMOVE_HOUSE, "Remove",
	...
	// Temple position
	sidesizer = newd wxStaticBoxSizer(
		newd wxStaticBox(this, wxID_ANY, "Brushes", wxDefaultPosition,
						 wxSize(150, 200)),
		wxVERTICAL);
	...
	tmpsizer = newd wxBoxSizer(wxHORIZONTAL);
	house_brush_button =
		newd wxToggleButton(this, PALETTE_HOUSE_BRUSH_BUTTON, "House tiles");
	tmpsizer->Add(house_brush_button);
	sidesizer->Add(tmpsizer, wxSizerFlags(1).Center());

	tmpsizer = newd wxBoxSizer(wxHORIZONTAL);
	select_position_button = newd wxToggleButton(
		this, PALETTE_HOUSE_SELECT_EXIT_BUTTON, "Select Exit");
...
Brush *HousePalettePanel::GetSelectedBrush() const {
	if (select_position_button->GetValue()) {
		...
		return (g_gui.house_exit_brush->getHouseID() != 0
					? g_gui.house_exit_brush
					: nullptr);
	} else if (house_brush_button->GetValue()) {
		g_gui.house_brush->setHouse(GetCurrentlySelectedHouse());
		return (g_gui.house_brush->getHouseID() != 0 ? g_gui.house_brush
													 : nullptr);
	}
	return nullptr;
}
...
bool HousePalettePanel::SelectBrush(const Brush *whatbrush) {
	...
	if (whatbrush->isHouse() && map) {
		const HouseBrush *house_brush =
			static_cast<const HouseBrush *>(whatbrush);
		for (HouseMap::iterator house_iter = map->houses.begin();
			...
			if (house_iter->second->id == house_brush->getHouseID()) {
				for (uint32_t i = 0; i < town_choice->GetCount(); ++i) {
					Town *town =
						reinterpret_cast<Town *>(town_choice->GetClientData(i));
					// If it's "No Town" (nullptr) select it, or if it has the
					// same town ID as the house
					if (town == nullptr ||
						town->getID() == house_iter->second->townid) {
						SelectTown(i);
						for (uint32_t j = 0; j < house_list->GetCount(); ++j) {
							if (house_iter->second->id ==
								reinterpret_cast<House *>(
									house_list->GetClientData(j))
									->id) {
								SelectHouse(j);
								return true;
							}
...
void HousePalettePanel::SelectTown(size_t index) {
	...
	if (map == nullptr || town_choice->GetCount() == 0) {
		...
		Town *what_town =
			reinterpret_cast<Town *>(town_choice->GetClientData(index));
		...
		ASSERT(what_town == nullptr || add_house_button->IsEnabled() ||
			   !IsEnabled());
	}
}
...
void HousePalettePanel::OnUpdate() {
	...
	if (map->towns.count() != 0) {
		...
		for (TownMap::iterator town_iter = map->towns.begin();
			...
			town_choice->Append(wxstr(town_iter->second->getName()),
								town_iter->second);
		}
...
void HousePalettePanel::OnClickAddHouse(wxCommandEvent &event) {
	...
	Town *town = reinterpret_cast<Town *>(
		town_choice->GetClientData(town_choice->GetSelection()));
...
void HousePalettePanel::OnClickEditHouse(wxCommandEvent &event) {
	...
	int selection = house_list->GetSelection();
	House *house =
		reinterpret_cast<House *>(house_list->GetClientData(selection));
	if (house) {
...
#ifdef __APPLE__
// On wxMac it is possible to deselect a wxListBox. (Unlike on the other
// platforms) EVT_LISTBOX is not triggered when the deselection is happening.
// http://trac.wxwidgets.org/ticket/15603 Here we find out if the listbox was
// deselected using a normal mouse up event so we know when to disable the
// buttons and brushes.
...
EditHouseDialog::EditHouseDialog(wxWindow *parent, Map *map, House *house)
	...
	name_field =
		newd wxTextCtrl(this, wxID_ANY, "", wxDefaultPosition, wxSize(230, 20),
						0, wxTextValidator(wxFILTER_ASCII, &house_name));
	...
	rent_field =
		newd wxTextCtrl(this, wxID_ANY, "", wxDefaultPosition, wxSize(160, 20),
						0, wxTextValidator(wxFILTER_NUMERIC, &house_rent));
	tmpsizer->Add(rent_field);
	id_field =
		newd wxTextCtrl(this, wxID_ANY, "", wxDefaultPosition, wxSize(70, 20),
						0, wxTextValidator(wxFILTER_NUMERIC, &house_id));
	...
	// House options
	guildhall_field =
		newd wxCheckBox(this, wxID_ANY, "Guildhall", wxDefaultPosition);

	sizer->Add(guildhall_field,
			   wxSizerFlags().Border(wxRIGHT | wxLEFT | wxBOTTOM, 20));
	...
	sizer->Add(tmpsizer, wxSizerFlags(1).Center().Border(
							 wxRIGHT | wxLEFT | wxBOTTOM, 20));
	...
	if (Validate() && TransferDataFromWindow()) {
		...
		if (new_house_rent < 0) {
			g_gui.PopupDialog(this, "Error",
							  "House rent cannot be less than 0.", wxOK);
			return;
		}
		...
		if (g_settings.getInteger(Config::WARN_FOR_DUPLICATE_ID)) {
			...
			for (HouseMap::const_iterator house_iter = houses.begin();
				 house_iter != houses.end(); ++house_iter) {
				House *house = house_iter->second;
				ASSERT(house);
				if (wxstr(house->name) == house_name &&
					house->id != what_house->id) {
					int ret =
						g_gui.PopupDialog(this, "Warning",
										  "This house name is already in use, "
